...
module.exports = NodeHelper.create({
    ...
    async _pollOnce (identifier, config) {
        ...
        try {
            ...
            let timeoutMs;
            if (mode === "local") {
                // Read timeout from config with sane defaults and minimum of 2000ms
                timeoutMs = Math.max(2000, Number((config && (config.captureTimeoutMs || config.offlineTimeout)) || 0) || 4000);
            } else {
